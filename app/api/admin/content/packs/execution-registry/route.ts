...
async function requireManager(permission: 'content:manage' | 'content:import' = 'content:manage') {
  ...
  if (!user) return { error: NextResponse.json({ error: 'Bạn cần đăng nhập trước khi quản trị pack execution registry.' }, { status: 401 }) };
  if (!assertLessonPermission(user, permission)) {
    return { error: NextResponse.json({ error: 'Bạn không có quyền quản trị pack execution registry.' }, { status: 403 }) };
  }
  return { user };
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await contentManagement.getAcademicPackExecutionRegistrySummary(auth.user, { packId });
...
export async function POST(request: NextRequest) {
  ...
  if (auth.error) return auth.error;
  const rate = assertRuntimeRateLimit(request, 'admin_admin_content_packs_execution_registry_write', { windowMs: 60_000, max: 20 });
  ...
  const result = apply
    ? await contentManagement.importAcademicPackExecutions({ payload, actorName: auth.user!.name, actorRole: auth.user!.role, packId, apply: true })
    : await contentManagement.previewAcademicPackExecutionRegistryImport(payload, { packId });
