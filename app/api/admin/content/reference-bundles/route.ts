...
async function requireManager(permission: 'content:manage' | 'content:import' = 'content:manage') {
  ...
  if (!user) return { error: NextResponse.json({ error: 'Bạn cần đăng nhập trước khi quản trị source bundle.' }, { status: 401 }) };
  if (!assertLessonPermission(user, permission)) {
    return { error: NextResponse.json({ error: 'Bạn không có quyền quản trị source bundle.' }, { status: 403 }) };
  }
  return { user };
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await contentManagement.getAcademicSourceBundleSummary(auth.user, { packId });
...
export async function POST(request: NextRequest) {
  ...
  if (auth.error) return auth.error;
  const rate = assertRuntimeRateLimit(request, 'admin_admin_content_reference_bundles_write', { windowMs: 60_000, max: 20 });
  ...
  const result = apply
    ? await contentManagement.importAcademicSourceBundles({ payload, actorName: auth.user!.name, actorRole: auth.user!.role, packId, apply: true })
    : await contentManagement.previewAcademicSourceBundleRegistryImport(payload, { packId });
